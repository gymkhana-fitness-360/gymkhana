...
async function getSetting(gymId: string, key: string): Promise<string | null> {
...
async function nextSequence(gymId: string, key: string, initial = 0): Promise<number> {
  const existing = await getSetting(gymId, key);
  const current = existing ? parseInt(existing, 10) : initial;
  const next = Number.isFinite(current) ? current + 1 : initial + 1;
  await prisma.setting.upsert({
    where: gymKeyWhere(gymId, key),
    create: { gymId, key, value: String(next) },
...
export async function getMemberCodePrefix(gymId: string): Promise<string> {
  return (await getSetting(gymId, memberCodePrefixKey(gymId))) || DEFAULT_MEMBER_PREFIX;
}

export async function getBillCodePrefix(gymId: string): Promise<string> {
  return (await getSetting(gymId, billCodePrefixKey(gymId))) || DEFAULT_BILL_PREFIX;
}
...
export async function generateNextMemberCode(gymId: string): Promise<string> {
  ...
  const seq = await nextSequence(gymId, memberCodeSequenceKey(gymId), 609);
  return `${prefix}${seq}`;
}
...
export async function generateNextBillNumber(gymId: string): Promise<string> {
  ...
  const seq = await nextSequence(gymId, billCodeSequenceKey(gymId), 0);
