  ...
  if (!gymId) {
    return {
      error: ApiErrors.validationError(
        "No gym selected or account has no locations.",
      ) as NextResponse,
    };
  }
...
export async function getWhatsappLifecycleTemplatesHandler(request: NextRequest) {
  try {
...
export async function putWhatsappLifecycleTemplatesHandler(request: NextRequest) {
  try {
    const ctx = await requireAdminGym(request);
    if ("error" in ctx && ctx.error) return ctx.error;
...
export async function postWhatsappLifecycleTemplatesHandler(request: NextRequest) {
  try {
    const ctx = await requireAdminGym(request);
    if ("error" in ctx && ctx.error) return ctx.error;
