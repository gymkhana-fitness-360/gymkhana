...
export async function listRemindersHandler(request: NextRequest) {
  try {
    ...
    const typeValidation = validateEnumParam(
...
export async function createReminderHandler(request: NextRequest) {
  try {
    const session = await auth();
    if (!session) {
      return ApiErrors.unauthorized();
